...
module.exports = function adminRouter(engine, wss) {
  ...
  // Permanently deletes a user and everything tied to them (wallets, ledger,
  // positions, pending email-verification tokens) via ON DELETE CASCADE — this
  // is NOT the same as suspend, which just locks the account without touching
  // any data. There is no undo for this.
  router.delete('/users/:id', (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (id === req.userId) return res.status(400).json({ error: "You can't delete your own account while logged in as it." });
